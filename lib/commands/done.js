    ...
    if (connections.size > 0) {
      ...
      ASYNC.eachSeries(memberDirs, (directory, callbackMain) => {
        ...
        processMessage.edit(logger.format('`[2/3]` Zipping the audio files...', logger.LoggerType.ANNOUNCE)).then((message) => {
          ...
          zipped.on('close', () => {
            let audioZipSize = fs.statSync(audioZip).size / 1000000
            let discordUpload = audioZipSize < 8.0

            processMessage.edit(logger.format('`[3/3]` Uploading' + (discordUpload ? ' to Discord' : ' to MEGA (this may take 10 seconds to a minute)') + '...', logger.LoggerType.ANNOUNCE)).then((message) => {
              try {
                if (discordUpload) {
                  (async function () {
                    await message.channel.sendFile(audioZip)
