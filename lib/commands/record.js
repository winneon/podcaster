      ...
      try {
        ...
        connection.on('speaking', (member, speaking) => {
          ...
          if (speaking) {
            ...
            let filename = `${timecode}`
            let stream = fs.createWriteStream(path.join(memberDir, filename + '.mp3'))

            stream.on('error', console.error)
            stream.on('close', () => {
              let newTimecode = Date.now()
              fs.renameSync(path.join(memberDir, filename + '.mp3'), path.join(memberDir, filename + '-' + newTimecode + '.mp3'))
            })
