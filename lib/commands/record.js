  ...
  async onCommand (bot, message, args) {
    ...
    if (channels.size > 0){
      try {
        ...
        let sent = await logger.announce(message.channel, 'Loading...')
        let connection = await channel.join()
        let fetched = await message.channel.fetchMessage(sent.id)

        fetched.edit(logger.format('Ready and listening.', logger.LoggerType.ANNOUNCE))
        ...
        console.error(err)
        logger.bare(message.channel, '```' + err + '```')
      }
    } else {
      logger.error(message.channel, 'You\'re not in a voice channel!')
    }
  }
}
