    ...
    if (connections.size > 0) {
      ...
      ASYNC.eachSeries(memberDirs, (directory, callbackMain) => {
        ...
        processMessage.edit(logger.format('`[2/3]` Zipping the audio files...', logger.LoggerType.ANNOUNCE)).then((message) => {
          ...
          zipped.on('close', () => {
            processMessage.edit(logger.format('`[3/3]` Uploading to MEGA...', logger.LoggerType.ANNOUNCE)).then((message) => {
              try {
                let storage = mega({
                  ...
                  if (err) {
                    ...
                    file.link((err, link) => {
                      if (err) {
                        ...
                        processMessage.edit(logger.format('Download: ' + link, logger.LoggerType.ANNOUNCE)).then((message) => {
